...
export default function Calendar() {
    ...
    const handleDate = (_, date) => setDate(date);


    // Read Todo
    const getTodo = async () => {
        const q = query(collection(firestore, "todos"), where("date", "==", date), where("createdBy.uid", "==", user.uid));
    ...
    return (
        <>
            <div className='text-center mt-3 mb-4'>
                ...
                <DatePicker className='select-date' name='selectDate' placeholder='Select Date' onChange={handleDate} />
